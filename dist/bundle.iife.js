    ...
    class KenshoForm extends Kensho {
        ...
        search() {
            ...
            for (const item of match) {
                ...
                if (unitElementsList[name] === undefined) {
                    unitElementsList[name] = {};
                }
                if (type === 'input') {
                    if (unitElementsList[name].input !== undefined) {
                        throw new Error(`The value "${name}" in ${prefix}name attr is duplicated.`);
                    }
                    unitElementsList[name].input = item;
                }
                else if (type === 'error') {
                    if (unitElementsList[name].error !== undefined) {
                        throw new Error(`The value "${name}.error" in ${prefix}name attr is duplicated.`);
                    }
                    unitElementsList[name].error = item;
                }
            }
            for (const [name, elements] of Object.entries(unitElementsList)) {
